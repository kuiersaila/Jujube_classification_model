...
class ResnetBlock(Model):
    ...
    def __init__(self, filters, strides=1, residual_path=False, BR_path=False, B0_path=False, R_path=False, B1_path=False):
    ...
    def call(self, inputs):
        ...
        if self.BR_path:
            ...
            out = y + residual  # 最后输出的是两部分的和，即F(x)+x或F(x)+Wx,再过激活函数
        return out
        ...
        for block_id in range(len(block_list)):  # 第几个resnet block
            for layer_id in range(block_list[block_id]):  # 第几个卷积层
                ...
                if block_id != 0 and layer_id == 0:  # 对除第一个block以外的每个block的输入进行下采样，第一个ResNet块为两条实线跳连
                    ...
                    block = ResnetBlock(self.out_filters, residual_path=True, B1_path=True)
                    ...
                    block = ResnetBlock(self.out_filters, residual_path=False, R_path=True)
                    ...
                    block = ResnetBlock(self.out_filters, residual_path=False, R_path=True, B0_path=True)
...
for content in contents:
    ...
    if int(pred) == int(value[1]):
        ...
        confusion_matrix[(int(pred), int(pred))] = confusion_matrix[(int(pred), int(pred))] + 1
        i = i + 1
    else:
        fp[int(pred)] = fp[int(pred)] + 1
        fn[int(value[1])] = fn[int(value[1])] + 1
        confusion_matrix[(int(pred), int(value[1]))] = confusion_matrix[(int(pred), int(value[1]))] + 1
    s = s + 1
    print(i, s)
r = float(i) / float(s)
