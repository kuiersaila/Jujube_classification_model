        ...
        for block_id in range(len(block_list)):  # 第几个resnet block
            for layer_id in range(block_list[block_id]):  # 第几个卷积层
                ...
                if block_id != 0 and layer_id == 0:  # 对除第一个block以外的每个block的输入进行下采样，第一个ResNet块为两条实线跳连
                    block = ResnetBlock(self.out_filters, strides=2, residual_path=True)  # 虚线连接residual_path=True，步长为2
                elif block_id != 0 and layer_id != 0:
                    block = ResnetBlock(self.out_filters, residual_path=False)  # 实线连接residual_path=False
                elif block_id == 0 and layer_id == 0:
                    block = ResnetBlock(self.out_filters, residual_path=True)
                else:
...
model_save_path = './checkpoint/ResNet50.ckpt'
